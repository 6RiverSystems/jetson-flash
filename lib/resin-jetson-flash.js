...
module.exports = class ResinJetsonFlash {
		...
		return new Bluebird(async (resolve, reject) => {
			...
			const child = spawn(
				...
				[
					...
					'./flash.sh',
					'-d',
					'/home/xeno/dev/l4t-dtb-compile-4-9-32-2/tegra186-tx2-6.dtb',
					'-o',
					'0x3090000',
                    '--no-reboot',
					'-c',
            ...
            // look at diff but this shouldn't clear the directory
    ...
    spawnDTBFlash(workPath) {
		return new Bluebird(async (resolve, reject) => {
            ...
            // look at diff but this should clear the directory if successful.
			child.on('error', reject);
			child.on('exit', resolve);
		});
	}
	...
	async generateArtifacts() {
        ...
        /// NOTE NOTE NOTE:
        //
        //   I added this because I made edits to the Nvidia sources
        //   and this was "helpfully" removing them.
        //
	...
	async run() {
        ...
        // Flash first, the device should be in recovery mode.
		await this.spawnFlash(workPath);
        // if flashing succeeds, prompt for second recovery mode.
        // then execute the following.
        await this.spawnDTBFlash(workPath);
	}
};
